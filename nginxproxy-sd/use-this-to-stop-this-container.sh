...
then
    docker-compose -f docker-compose.use-to-start-at-hurricane-electric-data-center.yml down
elif [ "${roleOfServer}"  = "backup-verification" ]
then
    docker-compose -f docker-composeuse-for-backup-verification-with-custom-ports.yml down
elif [ "${roleOfServer}"  = "alpha" ]
then
    docker-compose -f docker-compose-use-for-alpha.yml down
else
    docker-compose -f docker-compose.use-to-start-at-hurricane-electric-data-center.yml down
fi
